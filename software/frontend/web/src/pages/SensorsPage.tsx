...
const SensorsPage: React.FC<SensorsPageProps> = ({ onNavigateToDetail }) => {
  ...
  const [sensors, setSensors] = useState<Sensor[]>(() => {
    ...
    if (saved) {
      try {
        ...
        console.log('[SensorsPage] Initial state from localStorage:', parsed.length, 'sensors');
        return parsed;
      } catch (e) {
        console.error('[SensorsPage] Failed to parse initial state:', e);
        return [];
      }
    }
    console.log('[SensorsPage] No saved sensors, starting with empty array');
    return [];
  });
  ...
  useEffect(() => {
    const stored = localStorage.getItem('wit-sensors');
    console.log('[SensorsPage] Component mounted. localStorage:', stored ? 'has data' : 'empty');
