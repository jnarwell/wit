"""
W.I.T. Extended Database Models

File: software/backend/models/database_models_extended.py

Complete database models for the W.I.T. project management system
"""
...
task_dependencies = Table(
    ...
    Column('depends_on_id', UUID(as_uuid=True), ForeignKey('tasks.id')),
...
class User(Base):
    ...
    preferences = Column(JSONField(), default=dict)
    voice_settings = Column(JSONField(), default=dict)
    workspace_config = Column(JSONField(), default=dict)
    ...
    files_uploaded = relationship("ProjectFile", back_populates="uploaded_by_user")
...
class Equipment(Base):
    ...
    extra_data = Column(JSONField(), default=dict)
...
class TeamMember(Base):
    ...
    permissions = Column(JSONField(), default=list)
...
class Material(Base):
    ...
    properties = Column(JSONField(), default=dict)
...
class MaterialUsage(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    project_material_id = Column(UUID(as_uuid=True), ForeignKey("project_materials.id"), nullable=False)
...
class Comment(Base):
    ...
    commentable_id = Column(UUID(as_uuid=True))
